    ...
    dc.leafletChoroplethChart = function (parent, chartGroup) {
        ...
        _chart._postRender = function () {
			...
			_info.update = function (name) {
					this._div.innerHTML = (name ? name: "Hover and click to display");    // #EDIT
				};
            ...
            if (v && v.d) {
				...
				layer.on("mouseover",function(){
                    if(v!==undefined){		
						...
						var info =  pname[_chart.keyAccessor()(v.d)] + ": " + _chart.valueAccessor()(v.d).toLocaleString(); // #nb
            ...
            _update: function () {
                if (_parent.colorDomain()) { // check because undefined for marker charts
                    ...
                    for (i = 0; i < grades.length; i++) {
                        ...
                        this._div.innerHTML +=
						'<table><tr><td class="cblock" ><i style="background:' +  palette[i] + '"></i></td>' +
						'<td class="ltext">' + grades[i].toLocaleString() + (grades[i + 1] ? ' - ' + grades[i + 1].toLocaleString() + '<br>' : '+') + '</td></tr></table>';
